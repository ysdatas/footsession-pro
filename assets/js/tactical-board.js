...
const PROC = window.PROCEDURE_ID;
const CAN_EDIT = window.TB_CAN_EDIT;
...
function serialize() {
  return {
    ...
    nextNum: state.nextNum, nextOpp: state.nextOpp,
...
async function saveToDB(validate) {
  ...
  try {
    await API('php/api/tactical.php?action=save&procedure_id=' + PROC, {
      method: 'POST',
      body: { canvas_json: serialize(), canvas_image: exportClean(), vue_terrain: state.view },
    });
    persistLocal();
    if (validate) {
...
async function boot() {
  ...
  if (PROC) {
    try {
      const d = await API('php/api/tactical.php?action=load&procedure_id=' + PROC);
      $('#tbContext').textContent = 'schéma lié au procédé #' + PROC;
      if (d.schema && d.schema.canvas_json) { deserialize(JSON.parse(d.schema.canvas_json)); loaded = true; }
