...
async function loadVideos() {
  ...
  try {
    const { data: videos, error } = await sb.from('player_videos')
      .select('*, players(nom, prenom), video_views(watched_seconds, max_position_seconds, last_heartbeat_at)')
      .order('created_at', { ascending: false });
    if (error) throw error;

    document.getElementById('videosSub').textContent = `${videos.length} vidéo${videos.length > 1 ? 's' : ''} envoyée${videos.length > 1 ? 's' : ''}`;

    if (!videos.length) {
      tbody.innerHTML = `<tr><td colspan="8" class="text-muted" style="text-align:center;padding:24px;">Aucune vidéo envoyée pour le moment.</td></tr>`;
      return;
    }

    tbody.innerHTML = videos.map(v => {
      const views = v.video_views || [];
      const vu = views.length > 0;
      ...
      const derniere = views.reduce((d, x) => x.last_heartbeat_at && (!d || x.last_heartbeat_at > d) ? x.last_heartbeat_at : d, null);
    ...
    tbody.innerHTML = `<tr><td colspan="8" class="text-danger" style="text-align:center;padding:24px;">${escapeHtml(e.message)}</td></tr>`;
  }
}
